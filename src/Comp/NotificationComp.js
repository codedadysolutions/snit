...
export default class NotificationComp extends Component {
  ...
  render() {
      const{assign,onPressReject, onPressAccept, old}=this.props
    return (
        <View style={{height: old ? 110 : 160, width: width-20, borderRadius: 10, backgroundColor: '#DDD8F5', marginVertical: 10}}>
            <View style={{height: 50, borderBottomWidth: 1, borderColor: 'white', flexDirection:'row'}}>
                    ...
                    <View style={{flex:1, justifyContent: 'center', alignItems: 'flex-start', paddingHorizontal: 20}}>
                        <Text style={{color: '#3A4277', fontSize: 15, fontFamily: Font.FONT_FAMILY_BOLD}}>{assign ? 'Assign Request' : 'Leave Request'}</Text>
                    </View>

            </View>

            <View style={{height: 60,justifyContent: 'center', padding:10, paddingHorizontal: 20}}>
                <Text style={{color: '#3A3A3A', fontSize: 13, fontFamily: Font.FONT_FAMILY_REG}}>{assign ? 'You Recived a Assign Request from' : 'You Recived a Leave Request from'}<Text style={{color: '#3A4277', fontSize: 14, fontFamily: Font.FONT_FAMILY_BOLD}}> Ryan Saleh</Text>.</Text>
            </View>
